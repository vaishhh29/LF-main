...
const SignInScreen: React.FC = () => {
  ...
  useEffect(() => {
    GoogleSignin.configure({
      webClientId:
        '23692314207-72ohq3mc98t7hd08qved431058gm1n50.apps.googleusercontent.com',
      offlineAccess: true,
  ...
  const handleGoogleSignIn = async () => {
    try {
        ...
        .set(
          {
            ...
            photoURL: userSignIn.user.photoURL,
            provider: 'google',
      ...
      showError(`Welcome back, ${userSignIn.user.displayName}!`);
      setTimeout(() => {
      ...
      if (error?.code === statusCodes.SIGN_IN_CANCELLED) {
        ...
        showError('Sign-in already in progress. Please wait.');
      } else if (error?.code === statusCodes.PLAY_SERVICES_NOT_AVAILABLE) {
        showError('Google Play Services not available.');
      } else if (error?.message?.includes('DEVELOPER_ERROR') || error?.code === '10') {
        showError('Configuration error: Check Firebase SHA-1 setup.');
      } else {
        showError('Unable to sign in with Google. Please try again later.');
      }
    }
  };
...
const styles = StyleSheet.create({
  container: {
    ...
    paddingTop: height / 8,
    paddingBottom: 40, // ✅ ensures footer spacing
  },
  content: {
    flexShrink: 1,
  },
  ...
  footer: {
    marginBottom: 30, // ✅ footer spacing fixed
  },
  terms: {
    textAlign: 'center',
    color: '#909090',
