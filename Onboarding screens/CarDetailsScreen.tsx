...
// 🔥 MERGE all features (needed to save name + category)
const allFeatures = [
...
const CarFormScreen = () => {
  ...
  const handleSaveCarDetails = async () => {
    ...
    // Convert selected IDs → selected details
    ...
    try {
      // ✅ CORRECT PATH:
      // users/{uid}/cars/car1/carDetails
      await firestore()
        .collection("users")
        .doc(uid)
        .collection("cars")
        .doc("car1")
        .set(carDetailsData, { merge: true });

      console.log("🔥 Car details saved inside cars/car1");
                  ...
                  <View style={styles.stepRow}>
                    ...
                    {index < steps.length - 1 && (
                      <View
                        style={[
                          styles.stepLine,
                          { backgroundColor: isCompleted ? '#7A3EFF' : '#E8E8F8' },
                        ]}
                      />
                    )}
                  </View>
